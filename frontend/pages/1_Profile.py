...
if st.session_state.get("user") is None:
    ...
    with st.form("Skills"):
        ...
        if modify:
            temp_user["skills"] = [skill.strip() for skill in skills.split(",")]
    ...
    # if work history exists
        ...
        n = 0
        st.session_state["change_history"] = [False for _ in range(len(old_exps))]
        ...
        for idx, experience in enumerate(old_exps):
            with st.container():
                ...
                with st.form(f"Work History {n}"):
                    ...
                    start_date = st.date_input(bold("Start Date"),
                                            value=datetime.strptime(experience["start_date"], "%Y-%m-%d"))
                    end_date = st.date_input(bold("End Date"),
                                            value=datetime.strptime(experience["end_date"], "%Y-%m-%d"))
                    ...
                    if add and str(start_date) <= str(end_date):
                        ...
                        if old_exps[idx] != temp_user["work_history"][idx]:
                            ...
                            str(start_date) > str(end_date):
                        st.write(color("Can't end the job before you start it, no?", "red"))
                    if st.session_state.get("change_history")[idx] is True and st.session_state.get("deleted")[idx] is False:
                        st.write(color("Changes noted but not saved yet", "green"))
                    elif st.session_state.get("deleted")[idx] is True:
                        st.write(color("Scheduled for deletion but not saved yet.\
    ...
    with st.form("Change Password"):
        ...
        if change:
            ...
            if user["password"] == hash_password(confirm_password) and \
                ...
                st.session_state["change_password"] = False
                st.write(color("New password cannot be the same as the old", "red"))
    ...
    if save.button("Save Changes", type="primary"):
        try:
            ...
            updated_user = requests.get(f"{server}/users/fetch/{user_id}").json()
